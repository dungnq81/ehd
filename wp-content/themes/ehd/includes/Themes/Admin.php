...
if (!class_exists('Admin')) {
    class Admin
    {
        public function __construct()
        {
            ...
            add_action('admin_init', [&$this, 'ehd_admin_init'], 1);

            add_filter('admin_footer_text', [&$this, 'ehd_admin_footer_text']);
            add_action('admin_menu', [&$this, 'ehd_dashboard_meta_box'], 11);
            add_action('admin_enqueue_scripts', [&$this, 'ehd_admin_enqueue_scripts'], 31);
        ...
        public function term_header($columns)
        {
            if (class_exists('\ACF')) {
                ...
                // thumb
                $thumb = [
                    "term_thumb" => sprintf('<span class="wc-image tips">%1$s</span>', __("Thumb", 'ehd')),
                ];
                $columns = Arr::insertBefore('name', $columns, $thumb);

                // order
                $menu_order = [
                    'term_order' => sprintf('<span class="term-order tips">%1$s</span>', __("Order", 'ehd')),
                ];
        ...
        {
            $in = [
                "post_thumb" => sprintf('<span class="wc-image tips">%1$s</span>', __("Thumb", 'ehd')),
            ];
         ...
         * @return void
         */
        public function ehd_admin_enqueue_scripts()
        {
        ...
        public function ehd_admin_footer_text()
        {
