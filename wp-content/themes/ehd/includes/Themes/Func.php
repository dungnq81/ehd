...
if (!class_exists('Func')) {
    class Func
    {
        ...
        {
            $src = EHD_THEME_URL . '/assets/img/placeholder.png';
            if ($thumb) {
                $src = EHD_THEME_URL . '/assets/img/placeholder-320x320.png';
            }
            if ($img_wrap) {
        ...
        public static function breadcrumbs()
        {
            ...
            if (!is_home() && !is_front_page()) {
                ...
                echo '<li><a class="home" href="' . Url::home() . '">' . __('Home', 'ehd') . '</a></li>';
                ...
                elseif (is_search()) {
                    echo $before;
                    printf(__('Search Results for: %s', 'ehd'), get_search_query());
                    echo $after;
                } /** tag */
                elseif (is_tag()) {
                    echo $before;
                    printf(__('Tag Archives: %s', 'ehd'), single_tag_title('', false));
                    echo $after;
                } /** author */
                elseif (is_author()) {
                    global $author;
                ...
                if (get_query_var('paged')) {
                    echo '<li class="paged">';
                    echo ' (';
                    echo __('page', 'ehd') . ' ' . get_query_var('paged');
                    echo ')';
                    echo $after;
                }
        ...
        public static function menuFallback(?string $container = 'grid-container')
        {
            ...
            printf(
                __('Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'ehd'),
                /* translators: %s: menu url */
                sprintf(
                    __('<a class="_blank" href="%s">Menus</a>', 'ehd'),
                    get_admin_url(get_current_blog_id(), 'nav-menus.php')
                ),
                /* translators: %s: customize url */
                sprintf(
                    __('<a class="_blank" href="%s">Customize</a>', 'ehd'),
