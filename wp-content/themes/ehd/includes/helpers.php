...
if (!function_exists('safe_mailto')) {
     ...
     * @return string
     */
    function safe_mailto($email, string $title = '', $attributes = '')
    {
        if (trim($title) === '') {
            $title = $email;
        }
...
if (!function_exists('youtube_iframe')) {
    ...
    function youtube_iframe($url, int $autoplay = 0, bool $lazyload = true, bool $control = true)
    {
        ...
        if (isset($vars['v'])) {
            ...
            $_autoplay = 'autoplay=' . Cast::toInt($autoplay);
            ...
            $_src = 'https://www.youtube.com/embed/' . $idurl . '?wmode=transparent&origin=' . URL::home() . '&' . $_autoplay;
            $_control = '';
            if (!$control) {
...
if (!function_exists('if_empty')) {
     ...
     * @return mixed
     */
    function if_empty($value, $fallback, bool $strict = false)
    {
